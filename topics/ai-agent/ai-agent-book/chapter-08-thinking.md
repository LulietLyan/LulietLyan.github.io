...
先看能力支持，而不是先选算法。在留出任务上用与训练相近的采样配置测量 `pass@1`、`pass@k`、格式解析率和部分进展率，并对失败做归因。
...
切换门槛应可测：Mid-training 后，领域留出集改善、通用保留集可接受，并开始出现可验证的正确或部分正确轨迹；SFT 后，工具输出可稳定解析，再增加示范也难抬高 OOD；RL 前须已有可评分、偶尔成功的 rollout。`pass@k` 近零时直接上 RL，容易得到全零奖励、浪费采样预算。
