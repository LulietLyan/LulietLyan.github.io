...
筛选可依托遥操作记录本身（摄像头画面、操作输入、机械臂状态、动作耗时、抓取失败次数、重试次数、最终状态）：相对熟练操作者基线，自动剔除最终状态失败、重试过高、耗时异常偏长的轨迹；对仍通过的样本再做人工抽检。原则是先用可观测指标过滤，再进入训练，而不是假设“便宜数据越多越好”。
