...
> Redis 的 ZSet 在元素较少且较小时使用 `listpack`；超过配置阈值后使用跳表和字典两套结构。字典按 member 建索引，负责 O(1) 平均复杂度的成员存在性和 score 查询；跳表按 `(score, member)` 排序，负责 O(log N) 的插入、删除、排名和范围定位。
>
> 跳表的最底层包含全部节点，上层是按随机概率抽取的稀疏索引。查找时从最高层开始，能向右就向右，下一步会越过目标就下降一层，所以平均只需 O(log N)。插入时先记录每一层最后经过的前驱节点，再随机生成新节点层数，把各层指针接进去。
>
> Redis 的节点除了 `forward` 指针还有 `span`，表示当前指针跨过多少个底层节点。查找过程中累计 span 就能计算 `ZRANK`，按排名查询时也能跳过整段节点；底层节点还有 `backward` 指针，便于反向遍历。score 相同时再按 member 的字典序比较，保证顺序唯一、确定。
>
> Redis 选择跳表而不是只用平衡树，主要是实现简单、平均复杂度相同、范围遍历自然，而且增加 span 后可以高效支持排名。需要强调的是，ZSet 不是“只有一张跳表”：没有旁边的字典，按 member 查询 score 会退化成线性扫描。
...
插入不是找到一个位置后只改最底层指针。搜索过程中会维护两个数组：
